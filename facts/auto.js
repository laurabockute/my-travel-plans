Concrete.event.bind('btFacts.item.edit.open', function (options, settings) {
    ...
    var ftImageImage = $(newField).find('.ft-image-image-file-selector');
            if ($(ftImageImage).length > 0) {
                $(ftImageImage).concreteFileSelector({'inputName': $(ftImageImage).attr('data-file-selector-input-name'), 'filters': [], 'fID' : $(ftImageImage).attr('data-file-selector-f-id') });
            }
            var pageSelectorImage = $(newField).find('.ft-image-image-page-selector');
            if ($(pageSelectorImage).length > 0) {
                $(pageSelectorImage).concretePageSelector({'inputName': $(pageSelectorImage).attr('data-input-name'), 'cID' : $(pageSelectorImage).attr('data-cID')});
            }
    blockDesignerEditor('#' + ($(newField).find('textarea.ft-item-description_1').attr('id')).replace(/(:|\.|\[|\]|,|=|@)/g, "\\$1"));
    };
        ...
        if (deleteIt === true) {
            ...
            $(sortableItem).slideUp(slideSpeed, function () {
                if(typeof CKEDITOR !== 'undefined'){
                    var blockDesignerEditorInstance = CKEDITOR.instances[$(sortableItem).find('textarea.ft-item-description_1').attr('id')];
