Concrete.event.bind('btPageCarousel.item.edit.open', function (options, settings) {
    ...
    var updateItem = function (newField) {
        ...
        var ftImageImage = $(newField).find('.ft-image-image-file-selector');
            ...
            }$(newField).find('textarea.ft-wysiwyg').redactor({
    ...
    $(container).on('click', '.sortable-item .sortable-item-delete', function (e) {
        ...
        if (deleteIt === true) {
            $(this).parent().slideUp(slideSpeed, function () {
                $(this).remove();
                loadComplete();
            });
        }
