Concrete.event.bind('btFaqlaura.item.edit.open', function (options, settings) {
    ...
    blockDesignerEditor('#' + ($(newField).find('textarea.ft-item-content').attr('id')).replace(/(:|\.|\[|\]|,|=|@)/g, "\\$1"));
    };
        ...
        if (deleteIt === true) {
            ...
            $(sortableItem).slideUp(slideSpeed, function () {
                if(typeof CKEDITOR !== 'undefined'){
                    var blockDesignerEditorInstance = CKEDITOR.instances[$(sortableItem).find('textarea.ft-item-content').attr('id')];
